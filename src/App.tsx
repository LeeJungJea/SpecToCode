...
import { Check, Clipboard, Code2, FileJson2, Play, RotateCcw, Terminal, Wand2 } from 'lucide-react';
...
function CopyButton({ copied, onClick }: { copied: boolean; onClick: () => void }) { return <button type="button" className="inline-flex h-8 items-center gap-2 rounded-md border border-slate-700 bg-slate-950 px-3 text-xs font-semibold text-slate-300 transition hover:border-cyan-400 hover:text-cyan-200" onClick={onClick}>{copied ? <Check size={14} /> : <Clipboard size={14} />}{copied ? 'Copied' : 'Copy'}</button>; }
